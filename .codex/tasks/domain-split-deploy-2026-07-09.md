...
- Added `LINKETRY_WORKER_DOMAINS` so GitHub Actions can generate multiple Worker custom domains in `wrangler.toml`.
- Kept `LINKETRY_SHORT_DOMAIN` as a backward-compatible single-domain fallback.
...
- Bumped Linketry from `0.7.3` to `0.7.4`.

## Verification

- GitHub repository variables updated for split domains:
  - `LINKETRY_API_URL=https://go.y8o.de`
  - `LINKETRY_WORKER_DOMAINS=go.y8o.de,s.y8o.de`
  - `LINKETRY_SHORT_DOMAIN=go.y8o.de` as a legacy fallback
