...
test('versions initial JavaScript and CSS asset URLs without touching unrelated URLs', () => {
  ...
  assert.match(appendAssetCacheKey(html, '0.29.3'), /\/assets\/index-abc\.js\?v=0\.29\.3/);
  assert.match(appendAssetCacheKey(html, '0.29.3'), /\/assets\/index-def\.css\?v=0\.29\.3/);
  assert.doesNotMatch(appendAssetCacheKey(html, '0.29.3'), /inter\.woff2\?v=/);
});
...
test('replaces an existing asset query instead of stacking cache keys', () => {
  ...
  assert.equal(
    appendAssetCacheKey(html, '0.29.3'),
    '<script type="module" src="/assets/index-abc.js?v=0.29.3"></script>'
  );
});
