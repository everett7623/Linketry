...
- 新增统一版本中心，在同一界面展示当前版本、GitHub 最新版本、检查时间、在线升级能力和受保护部署进度
- 在线升级现在以“启动部署、等待队列、部署 Worker/Admin、验证运行版本与后台资源”四个阶段持续反馈，并保留 GitHub Actions 运行入口
- Settings 发布状态可直接进入版本中心，桌面侧栏、折叠侧栏和移动端共用同一升级状态

### Changed

- 顶部更新提示缩减为版本提醒和版本中心入口，确认、执行、失败恢复及部署进度统一由共享状态机管理
- 部署验证成功后保留短暂可见的完成状态，再自动刷新后台；传播延迟和手动刷新反馈继续跨页面刷新保存
- 同步所有 workspace、Worker 示例、Admin/Site 版本资产、部署文档与 CI fallback 至 v0.30.6

### Removed

- 移除侧栏通过 DOM 查询触发顶部升级按钮的组件耦合，以及不再使用的重复升级面板和操作组件
