...
const link = {
  id: 'link_1', slug: 'docs', domain: 'go.example.com', long_url: 'https://example.com/docs?a=%2f',
  title: 'Docs', description: null, tags: null, status: 'active', redirect_type: 302, clicks: 1,
  source: null, source_id: null, created_at: '2026-07-15T00:00:00.000Z',
  updated_at: '2026-07-15T00:00:00.000Z', last_clicked_at: null, expires_at: null,
  max_clicks: null, password_hash: null, password_protected: false, warning_enabled: 0,
  fallback_url: null, archived: 0,
};
...
test('filtered bulk UTM requires preview and downloads the confirmed change record', async ({ page }) => {
  ...
  await page.route('**/*', async (route) => {
    ...
    if (path === '/api/v1/auth/me') return route.fulfill({ status: 200, contentType: 'application/json', body: '{"success":true}' });
    ...
    return route.fulfill({ status: 404, contentType: 'application/json', body: '{"error":"missing mock"}' });
  });
  ...
  await expect(page.getByText(messages.en.bulkUtmPreviewSummary
    .replace('{total}', '1').replace('{ready}', '1').replace('{unchanged}', '0').replace('{invalid}', '0'))).toBeVisible();
  await page.getByRole('button', { name: messages.en.bulkUtmApply }).click();
  await expect(page.getByText(messages.en.bulkUtmComplete.replace('{changed}', '1').replace('{skipped}', '0'))).toBeVisible();
