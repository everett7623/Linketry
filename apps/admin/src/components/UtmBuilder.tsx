...
export function UtmBuilder({
  ...
  useEffect(() => {
    listUtmTemplates()
      ...
      .catch(() => toastError(t('utmTemplatesLoadFailed')));
  }, []);
  ...
  const saveCurrent = async () => {
    setBusy(true);
    try {
      const item = await createUtmTemplate(name, utm);
      setSaved((items) => [...items, item]);
      setSavedId(item.id);
      setName('');
      success(t('utmTemplateSaved'));
    } catch (e) {
      toastError(String(e));
    } finally {
      setBusy(false);
  ...
  const removeSaved = async () => {
    ...
    setBusy(true);
    try {
      await deleteUtmTemplate(savedId);
      setSaved((items) => items.filter((item) => item.id !== savedId));
      setSavedId('');
      success(t('utmTemplateDeleted'));
    } catch (e) {
      toastError(String(e));
    } finally {
      setBusy(false);
  ...
  return (
    <div className="space-y-3 border-t border-slate-800 pt-5">
      ...
      <div className="grid gap-2 sm:grid-cols-[1fr_auto]">
        <Select
          aria-label={t('savedUtmTemplates')}
          value={savedId}
          onChange={(e) => applySaved(e.target.value)}
          disabled={disabled}
        >
          <option value="">{t('savedUtmTemplates')}</option>
          {saved.map((item) => (
      ...
      <div className="grid gap-2 sm:grid-cols-[1fr_auto]">
        ...
        <Button
          ...
          size="sm"
          icon={<Save size={14} />}
          disabled={disabled || !name.trim() || saved.length >= 20}
          loading={busy}
          onClick={saveCurrent}
        >
          {t('saveTemplate')}
