...
export function RestoreBackupModal({ backup, open, onClose, onRestored }: RestoreBackupModalProps) {
  ...
  return (
    <Modal open={open && !!backup} onClose={onClose} title={t('restoreBackup')} size="xl">
      {backup && (
        <div className="space-y-5">
          <div>
            <p className="font-mono text-xs text-slate-300">{fileName(backup.filename)}</p>
            <p className="mt-1 text-sm text-slate-500">{t('restorePreviewHelp')}</p>
          </div>
            ...
            preview && (
              <>
                <div className="grid gap-3 sm:grid-cols-3">
                  <SummaryItem label={t('createAction')} value={preview.willCreate} />
                  <SummaryItem label={t('overwriteAction')} value={preview.willOverwrite} />
                  <SummaryItem label={t('renameAction')} value={preview.willRename} />
                  <SummaryItem label={t('skipAction')} value={preview.willSkip} />
                  <SummaryItem label={t('invalidAction')} value={preview.invalid} />
                  <SummaryItem label={t('rulesLabel')} value={preview.redirectRulesToRestore} />
                </div>
                      ...
                      {preview.preview.slice(0, 20).map((item) => (
                        <tr key={`${item.slug}-${item.action}-${item.nextSlug ?? ''}`}>
                          ...
                          <td className="px-3 py-2 text-slate-300">
                            {item.nextSlug ? `${item.action} -> ${item.nextSlug}` : item.action}
                          </td>
                          <td
                            className="max-w-sm truncate px-3 py-2 text-slate-500"
                            title={item.longUrl}
                          >
                            {item.longUrl}
                          </td>
