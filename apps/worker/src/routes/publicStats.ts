...
import {
  ...
  deletePublicStatsShare,
  findPublicStatsShare,
  listPublicStatsShares,
} from '../publicStats/index';
import { API_ROUTE_PREFIXES } from './apiVersion';

const routes = new Hono<{ Bindings: Env }>();

for (const apiPrefix of API_ROUTE_PREFIXES) {
  routes.get(`${apiPrefix}/public-stats/links/:id`, async (c) => {
    const authError = await requireAuth(c);
    if (authError) return authError;
    const share = (await listPublicStatsShares(c.env)).find(
      (item) => item.link_id === c.req.param('id')
    );
    return jsonOk(share ? publicConfig(share) : { enabled: false });
  });

  routes.post(`${apiPrefix}/public-stats/links/:id`, async (c) => {
  ...
  routes.delete(`${apiPrefix}/public-stats/links/:id`, async (c) => {
    const authError = await requireAuth(c, 'admin');
    if (authError) return authError;
...
function publicConfig(share: {
  ...
  return {
    enabled: true,
    days: share.days,
    show_countries: share.show_countries,
    show_referrers: share.show_referrers,
...
function renderPublicStats(
  ...
  share: { show_countries: boolean; show_referrers: boolean }
): string {
  const bars = summary.daily
    .map((item) => `<tr><td>${escape(item.date)}</td><td>${item.clicks}</td></tr>`)
    .join('');
  const list = (items: Array<{ label: string; clicks: number }>) =>
    `<ul>${items.map((item) => `<li><span>${escape(item.label)}</span><strong>${item.clicks}</strong></li>`).join('')}</ul>`;
