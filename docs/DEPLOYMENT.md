...
The post-deploy Admin readiness check is anonymous and read-only. It requests the exact asset cache keys used by browsers, without a cache-bypass header, and keeps the GitHub run active during Pages or custom-domain propagation. HTML fallbacks, incorrect MIME types, and missing or mismatched release cache keys fail readiness. A positive `max-age` is accepted only for the exact release-versioned asset URL. DNS-only Pages CNAMEs remain preferred so zone-level cache rules cannot retain an SPA fallback under an Admin asset URL.
