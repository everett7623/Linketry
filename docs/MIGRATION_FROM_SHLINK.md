...
This is the cutover process for moving from Shlink to Linketry.

## Safe Migration Plan

1. Keep Shlink running.
2. Export all short URLs from Shlink.
3. Deploy Linketry to a temporary test short-link domain, for example `go.example.com`.
4. Import Shlink data into Linketry.
5. Verify the import report.
6. Spot-check 30 to 50 old slugs.
7. Test high-traffic slugs.
8. Run Linketry in parallel for 1 to 2 weeks.
9. Back up Shlink.
10. Cut the old Shlink short domain DNS or Cloudflare routing over to Linketry.
11. Keep Shlink available for rollback for 1 to 2 weeks.

## Rollback

If Linketry has a production issue after cutover, point the old short domain back to Shlink immediately.
