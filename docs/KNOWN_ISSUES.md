...
跟踪当前 Linketry 未解决的 bug、限制和临时规避方案。
...
- **项**：v0.31.3 生产/Demo/官网 parity（✅ `go.uukk.de` / `demoapi.linketry.com` / `linketry.com` report 0.31.3）；GitHub Release `v0.31.3`；GitHub private vulnerability reporting（✅ enabled 2026-08-10）；keyboard/axe AT（✅ v0.31.3 `at-audit.spec.ts`）；screen-reader AT（pending）；fresh-account Quick Deploy rehearsal（证据写入 PROGRESS）；远程 D1 规模证据；Demo 可选 R2（Cloudflare `10042`）
...
- **修复版本**：v0.9.13
- **原影响**：单次导入超过 ~100 条 Shlink 数据时，`POST /api/v1/import/confirm` 可能触发前端 `AbortError`
...
1. 将输入解析也移到后台：
   - `POST /api/v1/import/confirm` 只接收 `content`、`source`、`filename`、`conflictStrategy`、`fieldMapping`
   - 立即创建 `status = 'pending'` 的 job，total 初始为 0
   - 通过 `ctx.waitUntil()` 在后台完成解析、校验、冲突检测和写入
2. 前端轮询 `/api/v1/import/jobs/:id` 时，total 会从 0 逐步更新到实际数量
3. 考虑为 `confirmImport` 单独设置更长的超时，或在解析阶段流式返回早期 job 信息

### 修复结果

- `POST /api/v1/import/confirm` 现在先创建 `pending` job（`total_count = 0`）并立即返回。
...
- **状态**：✅ Fixed in v0.9.16
- **影响来源**：Shlink API、Shlink 文件和 Linketry CSV 等所有共用导入写入流程的来源
- **现象**：任务先显示处理中，随后只写入约 73 条；换成之前从 Linketry 导出的 CSV 仍在相同位置停止。
...
- 使用用户提供的 195 行 Linketry CSV 在全新本地 D1 完成验证：首次导入成功 195、失败 0；再次导入跳过 195、覆盖 0。
...
## 3. 导入完成后 Admin 仍显示“导入中”（已修复）

- **状态**：✅ Fixed in v0.9.17
- **现象**：后台任务和导入历史已经显示完成，但当前导入按钮仍可能停留在“导入中”。
- **修复**：确认接口返回任务 ID 后立即查询状态；每次查询完成后再安排下一次查询；完成或失败时立即停止轮询并清空当前文件和预览。任务状态 API 的请求与响应同时设置为不缓存，避免读取旧的 `pending` 或 `processing` 状态。
